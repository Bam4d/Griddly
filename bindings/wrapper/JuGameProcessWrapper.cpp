...
namespace griddly {
class Ju_GameProcessWrapper {
 public:
  Ju_GameProcessWrapper(std::shared_ptr<Grid> grid, std::shared_ptr<Observer> observer, std::shared_ptr<GDYFactory> gdyFactory, std::string imagePath, std::string shaderPath)
  ...
  std::shared_ptr<Ju_StepPlayerWrapper> registerPlayer(std::string playerName, ObserverType observerType) {
    ...
    auto player = std::shared_ptr<Ju_StepPlayerWrapper>(new Ju_StepPlayerWrapper(nextPlayerId, playerName, observer, gdyFactory_, gameProcess_));
