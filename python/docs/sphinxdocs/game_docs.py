...
"""
    ...
    def _generate_game_doc(self, game_breakdown, relative_gdy_path):
        ...
        level_sphinx_string, level_images = self._generate_levels_description(
            game_breakdown
        )
        ...
        object_sphinx_string, object_images = self._generate_object_description(
            game_breakdown
        )
    ...
    def _generate_taster_image(self, game_breakdown):
        ...
        relative_image_path = os.path.join(
            "img", f'{name.replace(" ", "_")}-taster.png'
        )
        ...
        for category, games_in_category in self._game_documentation.items():
            ...
            for g, game_data in enumerate(games_in_category):
                ...
                for doc_image_filename, np_doc_image in doc_images.items():
                    ...
                    renderer.render(
                        np_doc_image.swapaxes(0, 2),
                        doc_sphinx_root.joinpath(doc_image_filename),
                    )
                ...
                taster_sphinx = taster_sphinx.replace(
                    "__relative__doc__link__",
                ...
                for taster_image_filename, np_taster_image in taster_images.items():
                    renderer.render(
                        np_taster_image.swapaxes(0, 2),
                        self._docs_root.joinpath(taster_image_filename),
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Automatically generate documentation for games."
    )
    parser.add_argument(
        "--filename-suffix",
        default=".yaml",
        help="Suffix for filenames to create documentation",
    )
