...
def multi_log_probs_from_logits_and_actions(policy_logits, actions, dist_class,
    ...
    log_probs = []
    if valid_action_trees is not None and False:
        T = valid_action_trees.shape[0]
        B = valid_action_trees.shape[1]
    # If we have valid action trees here its because we are using gridnet and we cn calculate logp
    # from actions in the tree to avoid huge amounts of unnecessary computation
        for i in range(len(policy_logits)):
            log_probs.append(torch.zeros([T, B]))

        grid_action_parts = len(model.grid_action_shape)

        for t in range(T):
            for b in range(B):
                x_tree = valid_action_trees[t,b]
                ...
                for x, y_tree in x_tree.items():
                    for y, subtree in y_tree.items():
                        ...
                        for a in range(grid_action_parts):
                            flattened_location = (model.width*y+x)*grid_action_parts+a
                            dist_part = policy_logits[flattened_location][t,b]
                            mask = torch.zeros([dist_part.shape[0]])
                            mask[subtree_options] = 1

                            print(f'pre-mask logits: {dist_part}')

                            logits = dist_part + torch.log(mask)
                            dist = Categorical(logits=logits)
                            action = actions[flattened_location][t,b]
                            print(f'mask: {mask}')
                            print(f'action: {action}')
                            print(f'post-mask logits: {logits}')
                            log_probs[flattened_location][t, b] = dist.log_prob(action)
...
def multi_from_logits(valid_action_trees,
    ...
    target_action_log_probs = multi_log_probs_from_logits_and_actions(
        target_policy_logits, actions, dist_class, model, valid_action_trees)
    ...
    if (len(behaviour_policy_logits) > 1
        ...
        behaviour_action_log_probs = multi_log_probs_from_logits_and_actions(
            behaviour_policy_logits, actions, dist_class, model, valid_action_trees)
