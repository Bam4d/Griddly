...
from griddly.util.rllib.torch.conditional_actions.conditional_action_mixin import (
...
def build_invalid_masking_vtrace_loss(policy, model, dist_class, train_batch):
    ...
    model_out += torch.maximum(
        torch.tensor(torch.finfo().min), torch.log(invalid_action_mask)
    )
...
ConditionalActionImpalaTrainer = ImpalaTrainer.with_updates(
    name="ConditionalActionImpalaTrainer",
