...
class TemporaryEnvironment:
    ...
    def render_rgb(self) -> npt.NDArray:
...
class EnvironmentBreakdown:
    def __init__(self, gdy_filename: str):
        """
    ...
    def _env(self, observer_name: str) -> TemporaryEnvironment:
    ...
    def _populate_common_properties(self) -> None:
        ...
        self.observer_configs: Dict[str, Any] = {
            "Block2D": {},
            "Sprite2D": {},
            "Vector": {},
            "Isometric": {},
        ...
        for observer_name in self.supported_observers:
            with self._env(observer_name) as env:
                ...
                for i, level in enumerate(self.levels):
                    env.game.load_level(i)
                    env.game.reset()
                    rendered_level = env.render_rgb()
                    self.levels[i]["Observers"][observer_name] = rendered_level
                    self.levels[i]["Size"] = [
                        env.game.get_width(),
                        env.game.get_height(),
                    ]
