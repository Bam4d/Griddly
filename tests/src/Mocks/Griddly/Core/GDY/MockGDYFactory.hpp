...
namespace griddly {
    ...
    public:
        MockGDYFactory()
                : GDYFactory(GDYFACTORY_ARGS(nullptr, nullptr)) {}
        ...
        MOCK_METHOD(std::shared_ptr<TerminationHandler>, createTerminationHandler,
                    (std::shared_ptr<Grid> grid, std::vector<std::shared_ptr<Player>> players), (const));
        MOCK_METHOD(std::shared_ptr<Observer>, createObserver,
                    (std::shared_ptr<Grid> grid, std::string observerName, uint32_t playerCount, uint32_t playerId),
                    ());
