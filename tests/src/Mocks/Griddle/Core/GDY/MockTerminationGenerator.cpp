#include <vector>

#include "Griddle/Core/GDY/TerminationGenerator.hpp"
#include "gmock/gmock.h"

namespace griddle {
class MockTerminationGenerator : public TerminationGenerator {
 public:
  MockTerminationGenerator() : TerminationGenerator(){};

  MOCK_METHOD(void, defineTerminationCondition, (TerminationState state, std::string commandName, std::vector<std::string> commandParameters), ());
  MOCK_METHOD(std::shared_ptr<TerminationHandler>, newInstance, (std::shared_ptr<Grid> grid, std::vector<std::shared_ptr<Player>> players), ());
};
}  // namespace griddle